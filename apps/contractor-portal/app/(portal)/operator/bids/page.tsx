      ...
      <div className="card">
        {bids.length === 0 ? (
          <EmptyState
            ...
            description="Send a bid invitation to contractors in your network to collect competitive proposals."
          />
        ) : (
          <table className="tbl">
            ...
            <tbody>
              {bids.map((b) => (
                <tr key={b.id}>
                  <td><div style={{ fontSize: 11, fontWeight: 600 }}>{b.jobName}</div><div style={{ fontSize: 10, color: "#6b6a66" }}>{b.trade}</div></td>
