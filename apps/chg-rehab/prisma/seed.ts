...
async function main() {
  ...
  // ── Investor Portal Phase 1 — demo investor + offerings ─────────────
  await seedInvestorPortal(company.id, properties);
...
async function seedInvestorPortal(
  ...
  // Best-effort Supabase auth user + user_profiles flag. Phase 1 ships
  // demo data even if Supabase is unreachable from the seed runner.
  ...
  if (sbUrl && sbKey) {
    try {
      ...
      // Find existing user (paginated search by email).
      ...
      // Upsert user_profiles row with is_investor=true. account_id is
      // intentionally left null — full login wiring (accounts row, RLS
      // bridge to a real Prisma Company) is Phase 2 of the portal.
      ...
      console.warn("[seed:investor] Supabase step failed:", (e as Error).message);
    }
  } else {
    console.warn(
  ...
  // ── Prisma: Investor + Offerings + Subscriptions + Distributions ─────
  ...
  const offeringSpecs = [
    {
      ...
      marketCity: "Norfolk",
      marketState: "VA",
      description: "Value-add single-family rehab + rent in Norfolk's Lakewood corridor.",
      targetIrrLow: "16.0",
      targetIrrHigh: "22.0",
      prefReturnPct: "8.0",
      holdMonths: 36,
  ...
  for (const o of offeringSpecs) {
    const propertyId = o.propertyCode ? properties[o.propertyCode] ?? null : null;
    const data = {
      companyId,
      propertyId,
      name: o.name,
  ...
  const updateSpecs = [
    ...
    },
    {
      ...
      body: "Soft commits crossed $1.9M of the $2.0M target. Hard close June 30, 2026.",
  ...
  // 4 Investor Activities (timeline)
  const activitySpecs = [
    {
      id: "seed-iact-1",
      ...
      relatedSubscriptionId: "seed-sub-1",
    ...
    },
    {
      ...
      relatedSubscriptionId: "seed-sub-4",
  ...
  for (const a of activitySpecs) {
    await prisma.investorActivity.upsert({
      ...
      update: {
        ...
        relatedSubscriptionId: a.relatedSubscriptionId ?? null,
  ...
  console.log(
    `[seed:investor] seeded investor ${INVESTOR_EMAIL} (${investorId}) with ` +
