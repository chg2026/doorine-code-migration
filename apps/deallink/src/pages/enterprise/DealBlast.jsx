  ...
  return (
    <Layout>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[['Open rate', '64%', 'text-green-400'], ['Reply rate', '12%', 'text-amber-400'], ['Total sent', '480', 'text-white'], ['Avg time-to-open', '4m', 'text-blue-400']].map(([l, v, c]) => (
          <Card key={l} className="p-4"><p className="text-slate-400 text-xs">{l}</p><p className={`text-2xl font-bold mt-1 ${c}`}>{v}</p></Card>
        ))}
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <span className="text-xs text-slate-400 flex items-center gap-1.5"><Sparkles className="w-3 h-3 text-amber-400" /> AI-matched 1,247 buyers</span>
        </CardHeader>
        <div className="divide-y divide-slate-700">
          {blasts.map((b) => (
            <div key={b.id} className="px-5 py-4 flex items-center gap-4 hover:bg-slate-800/30">
              <div className="w-10 h-10 rounded-lg bg-amber-400/10 flex items-center justify-center flex-shrink-0">
                {b.status === 'Sent' ? <CheckCircle2 className="w-4 h-4 text-green-400" /> : <Clock className="w-4 h-4 text-amber-400" />}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-white text-sm font-medium truncate">{b.deal}</p>
                <p className="text-slate-500 text-xs">{b.when}</p>
              </div>
...
function Stat({ l, v, c = 'text-white' }) { return <div className="text-right"><p className="text-slate-500">{l}</p><p className={`font-semibold ${c}`}>{v}</p></div>; }
